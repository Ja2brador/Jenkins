...
+!setup_priorities
    <-  +task_priority("TASK_NONE",0);
        ...
        +task_priority("TASK_GIVE_AMMOPAKS", 1200);
        +task_priority("TASK_GIVE_BACKUP", 0);
        +task_priority("TASK_GET_OBJECTIVE",0);
        +task_priority("TASK_ATTACK", 5001);
...
+!update_targets
	<-	?debug(Mode); if (Mode<=1) { .println("TARGET FINISHED!!") }.
...
+!init
   <- 	//?setup_priorities;
   		...
   		.delete(task(_, "TASK_GET_OBJECTIVE", _, _, _),TasksSorted,NewTaskList);
   		-+tasks(NewTaskList);
   		!add_task(task(7000,"TASK_GOTO_POSITION", M, pos(155, 0, 100),""));
		!add_task(task(3005, "TASK_ATTACK", M, pos(142,0,104),"")).
