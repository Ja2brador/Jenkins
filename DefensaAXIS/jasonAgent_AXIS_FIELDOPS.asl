...
+!perform_aim_action
    <-  // Aimed agents have the following format:
        ...
        if (AimedAgentTeam == 100) {
			...
			!add_task(task(750, "TASK_GOTO_POSITION", M, NewDestination, ""));
        }
        .
...
+!setup_priorities
    <-  +task_priority("TASK_NONE",0);
        ...
        +task_priority("TASK_ATTACK", 2001);
...
+!update_targets 
	...
	!add_task(task(0, "TASK_PATROLLING", M, pos(ObjectiveX, ObjectiveY, ObjectiveZ), "")).
...
+!init
  ...
  if(Zb<Za){	//si esta mas cerca del punto de abajo
  	.concat(TaskList, [task(3000, "TASK_GOTO_POSITION", M, pos(175, 0, 115), "")], NewT1);           
    +tasks(NewT1);
  }
  else{
  	.concat(TaskList, [task(3000, "TASK_GOTO_POSITION", M, pos(175, 0, 115), "")], NewT1);           
